...
const StructRuleDetailPage: FC = () => {
  ...
  const edit = (
    params:
      | {
          type: 'category';
          record: Partial<StructRule.Category>;
        }
      | {
          type: 'field';
          record: Partial<StructRule.Field>;
        },
  ) => {
    form.setFieldsValue({ ...params.record });
    setEditingKey(`${params.type}-${params.record.uid}`);
  };
  const cancelEdit = () => setEditingKey('');
  const save = async (type: 'category' | 'field', key: string) => {
    try {
      const detail = await form.validateFields();
      console.log('保存编辑内容:', detail);
      ...
      //       ? newData.fields
      //       : [];
      // const index = list.findIndex((item) => key === item.uid);
      // if (index > -1) {
  ...
  const categoryColumns = [
    ...
    },
    {
      title: '操作',
      dataIndex: 'operation',
      render: (_, record: StructRule.Category) => {
        const editable = isEditing(`category-${record.uid}`);
        return editable ? (
          <span>
            <Typography.Link
              onClick={() => save('category', record.uid)}
          ...
          <Typography.Link
            disabled={editingKey !== ''}
            onClick={() => edit({ type: 'category', record })}
          >
            Edit
          </Typography.Link>
        );
  ...
  const categoryColumnsMerged: TableProps<StructRule.Category>['columns'] =
  ...
  const fieldColumns = [
    ...
    },
    {
      title: '操作',
      dataIndex: 'operation',
      render: (_, record: StructRule.Field) => {
        const editable = isEditing(`field-${record.uid}`);
        return editable ? (
          <span>
            <Typography.Link
              onClick={() => save('field', record.uid)}
              style={{ marginInlineEnd: 8 }}
            >
              Save
            </Typography.Link>
          ...
          <Typography.Link
            disabled={editingKey !== ''}
            onClick={() => edit({ type: 'field', record })}
          >
            Edit
          </Typography.Link>
        );
  ...
  const fieldColumnsMerged: TableProps<StructRule.Field>['columns'] =
        ...
        <div className="flex h-full">
          <div className="flex-1 w-[calc(100%_-_200px_-_200px_-_24px)] overflow-auto">
            ...
            <Card title="明细字段" className="mt-[12px]">
              <Table<StructRule.Field>
                dataSource={detail.fields}
                columns={fieldColumnsMerged}
              />
